...
std::string write_q134_model() {
  const std::string model_path =
      (std::filesystem::temp_directory_path() / "cuda_q134_batch_force.nep").string();
...
Prediction evaluate(NepaModel* model, const std::vector<double>& positions) {
  ...
  if (status != NEPA_STATUS_OK) {
    std::cerr << "CUDA q134 force status=" << status << "\n";
...
int main() {
  ...
  const std::string model_path = write_q134_model();
  ...
  for (std::size_t coordinate = 0; coordinate < positions.size(); ++coordinate) {
    ...
    if (diff > tolerance) {
      std::cerr << "q134 finite-difference mismatch coordinate="
