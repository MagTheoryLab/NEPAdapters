...
std::string write_angular_model() {
  const std::string model_path =
      (std::filesystem::temp_directory_path() / "cuda_angular_batch_force.nep").string();
...
Prediction evaluate(NepaModel* model, const std::vector<double>& positions) {
  ...
  if (status != NEPA_STATUS_OK) {
    std::cerr << "CUDA angular force status=" << status << "\n";
...
int main() {
  ...
  const std::string model_path = write_angular_model();
  ...
  for (std::size_t coordinate = 0; coordinate < positions.size(); ++coordinate) {
    ...
    if (diff > tolerance) {
      std::cerr << "angular force finite-difference mismatch coordinate="
