...
std::string write_q123_model() {
  const std::string model_path =
      (std::filesystem::temp_directory_path() / "cuda_q123_batch_force.nep").string();
...
Prediction evaluate(NepaModel* model, const std::vector<double>& positions) {
  ...
  if (status != NEPA_STATUS_OK) {
    std::cerr << "CUDA q123 force status=" << status << "\n";
...
int main() {
  ...
  const std::string model_path = write_q123_model();
  ...
  for (std::size_t coordinate = 0; coordinate < positions.size(); ++coordinate) {
    ...
    if (diff > tolerance) {
      std::cerr << "q123 finite-difference mismatch coordinate="
