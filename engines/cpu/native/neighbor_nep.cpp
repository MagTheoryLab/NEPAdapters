  ...
  std::vector<double>& r12)
{
  ...
  constexpr int neighbor_cell_count = 125;
  std::vector<int> cellNeighborCells(
    static_cast<std::size_t>(numCells[3]) * neighbor_cell_count);
  for (int cell_index = 0; cell_index < numCells[3]; ++cell_index) {
    const int cx = cell_index % numCells[0];
    const int cy = (cell_index / numCells[0]) % numCells[1];
    const int cz = cell_index / (numCells[0] * numCells[1]);
    int neighbor_count = 0;
    for (int k = -2; k <= 2; ++k) {
      int nz = cz + k;
      if (nz < 0) {
        nz += numCells[2];
      } else if (nz >= numCells[2]) {
        nz -= numCells[2];
      }
      for (int j = -2; j <= 2; ++j) {
        int ny = cy + j;
        ...
        for (int i = -2; i <= 2; ++i) {
          ...
          if (nx < 0) {
            nx += numCells[0];
          } else if (nx >= numCells[0]) {
            nx -= numCells[0];
          }
          cellNeighborCells[
            static_cast<std::size_t>(cell_index) * neighbor_cell_count + neighbor_count++] =
            nx + numCells[0] * (ny + numCells[1] * nz);
        }
      }
    }
  }
  ...
  for (int n1 = 0; n1 < N; ++n1) {
    ...
    const int atom_cell = atomCell[n1];
    const int* neighborCells =
      cellNeighborCells.data() + static_cast<std::size_t>(atom_cell) * neighbor_cell_count;
    for (int neighbor_index = 0; neighbor_index < neighbor_cell_count; ++neighbor_index) {
      const int neighborCell = neighborCells[neighbor_index];
  ...
  const bool report_required_capacity)
{
  ...
  double* g_y12_angular = r12.data() + size_x12 * 4;
  double* g_z12_angular = r12.data() + size_x12 * 5;
  const double rc_radial_sq = rc_radial * rc_radial;
  const double rc_angular_sq = rc_angular * rc_angular;

  const int num_images = num_cells[0] * num_cells[1] * num_cells[2];
  std::vector<ImageShift> image_shifts(num_images);
  int image_count = 0;
  for (int ia = 0; ia < num_cells[0]; ++ia) {
    for (int ib = 0; ib < num_cells[1]; ++ib) {
      for (int ic = 0; ic < num_cells[2]; ++ic) {
        double delta_x = box[0] * ia + box[1] * ib + box[2] * ic;
        double delta_y = box[3] * ia + box[4] * ib + box[5] * ic;
        double delta_z = box[6] * ia + box[7] * ib + box[8] * ic;
        image_shifts[image_count++] = {
          ebox[9] * delta_x + ebox[10] * delta_y + ebox[11] * delta_z,
          ebox[12] * delta_x + ebox[13] * delta_y + ebox[14] * delta_z,
          ebox[15] * delta_x + ebox[16] * delta_y + ebox[17] * delta_z,
          ia == 0 && ib == 0 && ic == 0};
      }
    }
  }
...
#pragma omp parallel for
  ...
  for (int n1 = 0; n1 < N; ++n1) {
    ...
    for (int n2 = 0; n2 < N; ++n2) {
      ...
      double dy = g_y[n2] - y1;
      double dz = g_z[n2] - z1;
      double sx_base = ebox[9] * dx + ebox[10] * dy + ebox[11] * dz;
      double sy_base = ebox[12] * dx + ebox[13] * dy + ebox[14] * dz;
      double sz_base = ebox[15] * dx + ebox[16] * dy + ebox[17] * dz;
      for (const ImageShift& image : image_shifts) {
        if (image.is_origin && n1 == n2) {
          continue; // exclude self
        }

        double sx12 = sx_base + image.sx;
        double sy12 = sy_base + image.sy;
        double sz12 = sz_base + image.sz;
        applyMicOne(sx12);
        applyMicOne(sy12);
        applyMicOne(sz12);

        double x12 = ebox[0] * sx12 + ebox[1] * sy12 + ebox[2] * sz12;
        double y12 = ebox[3] * sx12 + ebox[4] * sy12 + ebox[5] * sz12;
        double z12 = ebox[6] * sx12 + ebox[7] * sy12 + ebox[8] * sz12;
