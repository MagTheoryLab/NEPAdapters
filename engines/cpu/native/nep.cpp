...
void NEP::init_from_file(const std::string& potential_filename, const bool is_rank_0)
{
  ...
  auto parse_spin_line = [&](const std::vector<std::string>& spin_tokens) {
    ...
    if (spin_tokens[0] == "spin_baseline") {
      ...
      if (paramb.spin_basis_size < 0 || angular < 0) {
  ...
  std::vector<double>* descriptor)
{
  ...
  find_neighbor_list_small_box(
    paramb.rc_radial_max, paramb.rc_angular_max, N, batch_neighbor_capacity, box, position,
  ...
  std::vector<double>& descriptor)
{
  ...
  allocate_memory(N);
  find_neighbor_list_small_box(
    paramb.rc_radial_max, paramb.rc_angular_max, N, batch_neighbor_capacity,
