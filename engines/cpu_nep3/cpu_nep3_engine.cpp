...
class CpuNep3Model : public nep_adapters::Model {
 public:
  explicit CpuNep3Model(const std::string& model_path) : nep_(model_path) {}
...
class CpuNep3Engine : public nep_adapters::Engine {
 public:
  ...
  NepaStatus load_model(
    ...
    try {
      out = std::make_unique<CpuNep3Model>(model_path);
      return NEPA_STATUS_OK;
    } catch (const std::exception&) {
      out.reset();
      return NEPA_STATUS_RUNTIME_ERROR;
