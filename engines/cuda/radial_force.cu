...
__global__ void accumulate_lammps_radial_forces(
  ...
  for (int slot = 0; slot < nn_radial[atom]; ++slot) {
    ...
    atomicAdd(&force_soa3[atom_stride + neighbor], -static_cast<double>(f12y));
    atomicAdd(&force_soa3[2 * atom_stride + neighbor], -static_cast<double>(f12z));
...
void accumulate_lammps_radial_forces_on_device(
  ...
  const int blocks = (atom_count + threads - 1) / threads;
  if (blocks > 0) {
    accumulate_lammps_radial_forces<<<blocks, threads>>>(
        atom_count,
        static_cast<int>(view.atom_capacity),
        protocol.num_types,
        protocol.n_max_radial,
...
void accumulate_radial_forces_batched(
  ...
  const int blocks = (atom_count + threads - 1) / threads;
  if (blocks > 0) {
    accumulate_radial_forces_batched<<<blocks, threads>>>(
        atom_count,
        static_cast<int>(view.atom_capacity),
        protocol.num_types,
        protocol.n_max_radial,
