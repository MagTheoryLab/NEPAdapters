# cpu_opt Performance Maintenance

`cpu_nep3` is the reference/oracle. Do not move optimization experiments into
that engine. `cpu_opt` owns optimized CPU behavior and must keep passing parity
tests against `cpu_nep3`.

## Default Path

Keep these optimizations in the default `cpu_opt` path:

- symbol-isolated `cpu_opt` engine built from the optimized NEP CPU source;
...
- CBLAS/Accelerate-backed batch ANN when a provider is available;
...
For performance readout, compare `cpu_nep3` and `cpu_opt` with the NEP89
benchmark in both modes:

```sh
OMP_NUM_THREADS=4 VECLIB_MAXIMUM_THREADS=1 \
  .build/cpu-opt-clean/benchmarks/nep_adapters_bench_cpu_nep89 \
  --engine cpu_opt --mode lammps --replicate 4x4x2 --rank-grid 2x1x1 \
...
without expanding `cpu_nep3` or adding more experimental `cpu_opt` switches.
